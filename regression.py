...
class LinearRegressionModel:
    ...
    def train(self, X_revenue, y_revenue, mode):
        X_revenue = LoadData.modify_data(X_revenue, self.mats, mode)
        X_revenue = X_revenue.drop('status', axis='columns')
        self.X = X_revenue
        # self.vote_model.fit(X_vote, y_vote)
...
def train_and_plot_mse(train_X, train_y, test_X, test_y, mats, name, mode):
    percent_start = 30
    run_range = range(percent_start, 101, 20)
